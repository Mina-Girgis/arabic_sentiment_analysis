...
class TransformerModel:
    def __init__(self, train_data, test_data, max_words, max_len, epochs):
        ...
        self.num_attention_heads = 8  # Number of attention heads in the MultiHeadAttention layer
        self.ff_dim = 8  # Number of attention heads in the MultiHeadAttention layer
        self.num_blocks = 4  # Number of Transformer blocks
    ...
    def compile_model(self, training):
        self.model = self.build_transformer(self.embedding_dim, self.num_attention_heads, self.ff_dim, self.num_blocks,
                                            self.dropout_rate, training)
        self.model.compile(optimizer=Adam(learning_rate=self.ETA), loss='sparse_categorical_crossentropy',
                           metrics=['accuracy'])
